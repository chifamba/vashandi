...
| 3 | Core Server Implementation (HTTP) | In Progress (Router initialized, Health, Dashboard, Activity routes ported) |
| 4 | WebSockets and Realtime Functionality | Not Started |
| 5 | Adapters and Plugins Architecture | Not Started |
| 6 | CLI Porting | In Progress (Cobra initialized, Doctor and Onboard commands stubbed) |
...
*   **Phase 6 (CLI):** The `doctor` and `onboard` CLI commands have been temporarily stubbed with basic print statements rather than fully implementing all config checks and interactive prompts, as the underlying configuration-loading logic needs to be properly architected in Go first.
